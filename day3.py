...
system = """You are an assistant for question-answering tasks.
    ...
    At the end, include your sources in the format (Source: Title(URL)).
    """
...
def web_search(state):
    ...
    web_results = "\n".join([d["content"] + "[출처:%s(%s)" % (d["title"], d["url"]) for d in docs])
...
def check_hallucination(state):
    global check_hallucinated_count
    """
    ...
    print("---CHECK HALLUCINATIONS---")
    if check_hallucinated_count >= 1:
        print("WITH RETRY")

    check_hallucinated_count += 1
    question = state["question"]
    documents = state["documents"]
    generation = state["generation"]
...
# Build graph
# Docs Retrieval(retrieve) -> Relevance Checker(grade_documents)

workflow.add_edge(START, "retrieve")

workflow.add_edge("retrieve", "grade_documents")

# Relevance Checker(grade_documents) -> Generate Answer(generate) or Search Tavily(websearch)
...
# Search Tavily(websearch) -> Relevance Checker(grade_documents)

workflow.add_edge("websearch", "grade_documents")

# Generate Answer(generate) -> Hallucination Checker(check_hallucination) -> Answer to User(END) or Generate Answer(generate)
...
if generate_report:
    with st.spinner("Generating Report"):
        ...
        final_report = value.get("generation", "")
